 ...
 * @author Reid
 */
public class PunchInAction extends AbstractJSONAction {
    ...
    public String execute() throws Exception {
        ...
        try {
            TiimeclockBDFactory.factory.build(this.getSessionContext()).punchIn(
                    this.getSessionContext().getUser().getUser(), this.getSessionRemoteAddress());
            this.setTimeclockResult("Punch in succeeded.");
        } catch (TimeclockBDException e) {
            this.setTimeclockResult("Unable to punch in.");
        }

        DashboardMiscellaneousRecentAction a = new DashboardMiscellaneousRecentAction();
        a.setTypeName("Time clock: ");
        a.setDescription("You have punched in.  Time: " + SimpleDateFormat.getDateTimeInstance().format(this.getSessionContext().getResolvedNow()));
